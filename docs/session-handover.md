...
**タスク内のチェック項目機能**のこと。データベーステーブル名は `subtasks` だが、UI上では「チェックリスト」と表現する。
...
- タスクの中に複数のチェック項目を持てる
- 各項目は完了/未完了をチェックできる
- **進捗率の自動計算**: チェックリストの完了率がタスクの進捗率になる
  - 例: 4個中2個完了 → 進捗率50%
...
- ✅ CRUD関数の実装
  ...
  - チェックリスト管理: `lib/subtasks.ts`（※テーブル名はsubtasks、UI上はチェックリスト）
...
│   ├── tasks.ts            # タスクCRUD
│   ├── members.ts          # メンバーCRUD
│   └── subtasks.ts         # チェックリストCRUD（※テーブル名はsubtasks）
├── types/
...
#### Week 1-2: タスク一覧・詳細モーダル
1. **タスク一覧ページ**
   - データ取得とテーブル表示
   - フィルター・ソート機能
   - タスク作成ボタン

2. **タスク詳細モーダル**（※個別ページではなくモーダル）
   - タスク情報表示・編集
   - チェックリスト機能（完了/未完了チェック）
   - 進捗率の自動計算
   - 担当者アサイン（メイン/フォロー）
...
4. **subtasks** - チェックリスト項目（※UI上は「チェックリスト」と表現）
